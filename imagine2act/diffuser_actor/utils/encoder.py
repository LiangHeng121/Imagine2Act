...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.transform_scalar_encoder = nn.Sequential(
            nn.Linear(1, embedding_dim),
            nn.ReLU(),
            nn.Linear(embedding_dim, embedding_dim)
        )
        self.transform_len_agg = nn.Linear(12, 1)
    ...
    def encode_images(self, rgb, pcd):
        ...
        Args:
            - rgb: (B, ncam, 3, H, W) or (B, ncam, 2, 3, H, W), pixel intensities
            - pcd: (B, ncam, 3, H, W) or (B, ncam, 2, 3, H, W), positions

        Returns:
            - rgb_feats_pyramid: [(B, ncam, F, H_i, W_i)]
            - pcd_pyramid: [(B, ncam * H_i * W_i, 3)]
        """
        return self._encode_single_frames(rgb, pcd)
    
    def _encode_single_frames(self, rgb, pcd):
        """
