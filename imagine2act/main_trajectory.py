    ...
    def evaluate_nsteps(self, model, criterion, loader, step_id, val_iters,
        ...
        for i, sample in enumerate(loader):
            ...
            action = model(
                ...
                run_inference=True,
                gt_matrix=sample["matrix"].to(device),
...
def traj_collate_fn(batch):
    keys = [
        ...
        "rgbs", "pcds",
        "curr_gripper", "curr_gripper_history", "action", "instr", "matrix", "edge_mask"
    ]
    ret_dict = {
        key: torch.cat([
            item[key].float() if key != 'trajectory_mask' else item[key]
            for item in batch if key in item
        ]) for key in keys if any(key in item for item in batch)
    }
