...
ALL_RLBENCH_TASKS = [
    ...
    'water_plants', 'wipe_desk',"put_knife_in_knife_block","put_toilet_roll_on_stand","put_plate_in_colored_dish_rack"
]
...
class Actioner:
    ...
    def load_episode(self, task_str, variation, env, current_demo_id=None):
        ...
        goal_root = f"{self._root}/{task_str}/variation0/episodes/episode{current_demo_id}/imagine/"
        rgb_path = os.path.join(goal_root, "fused_scene_rendered.png")
        pcd_path = os.path.join(goal_root, "fused_scene_filtered_dense_point_image_world.npy")

        # Check existence of the imagined goal frame
        if not os.path.exists(rgb_path) or not os.path.exists(pcd_path):
            raise FileNotFoundError(f"Imagined goal frame not found: {rgb_path} or {pcd_path}")

        # Load goal RGB image (BGR -> RGB -> normalized tensor)
        goal_img = cv2.imread(rgb_path)                              # (H, W, 3), BGR
        goal_img = cv2.cvtColor(goal_img, cv2.COLOR_BGR2RGB)         # Convert to RGB
        goal_img = torch.from_numpy(goal_img).permute(2, 0, 1).float() / 255.0  # (3, H, W)

        # Load goal point cloud (dense, world coordinates)
        goal_pcd = np.load(pcd_path)                                 # (H, W, 3), float32
        goal_pcd = torch.from_numpy(goal_pcd).permute(2, 0, 1)       # (3, H, W)

        # Add batch dimension
        self._goal_rgb = goal_img.unsqueeze(0)                       # (1, 3, H, W)
        self._goal_pcd = goal_pcd.unsqueeze(0)                       # (1, 3, H, W)

        print(f"Loaded imagined goal frame: {rgb_path}")

        # Load transformation matrix of the imagined goal
        matrix_path = os.path.join(goal_root, "transform.txt")
        ...
        self._matrix = torch.from_numpy(matrix).float().unsqueeze(0)  # (1, 4, 4)
    ...
    def predict(self, rgbs, pcds, gripper,
        ...
        goal_rgb = self._goal_rgb.to(rgbs.device)
        goal_pcd = self._goal_pcd.to(pcds.device)

        # rgbs: (bs, num_hist, num_cameras, 3, H, W) -> (bs, num_hist, num_cameras + 1, 3, H, W)
        bs, num_hist, num_cameras = rgbs.shape[:3]

        # Expand the imagined goal frame for each timestep as an additional camera view
        goal_rgb_expanded = goal_rgb.unsqueeze(0).unsqueeze(0).repeat(bs, num_hist, 1, 1, 1, 1)
        goal_pcd_expanded = goal_pcd.unsqueeze(0).unsqueeze(0).repeat(bs, num_hist, 1, 1, 1, 1)

        # Concatenate goal view to RGB and point cloud sequences
        rgbs = torch.cat([rgbs, goal_rgb_expanded], dim=2)
        pcds = torch.cat([pcds, goal_pcd_expanded], dim=2)

        # Repeat transformation matrix for each batch
        matrix = self._matrix.to(rgbs.device).repeat(bs, 1, 1)
...
class RLBenchEnv:
    ...
    def _evaluate_task_on_one_variation(
        ...
        for demo_id in range(num_demos):
            ...
            actioner.load_episode(task_str, variation, self, demo_id)
            ...
            for step_id in range(max_steps):
                ...
                if save_frames and save_dir is not None:
                    self._save_current_frame(rgb, task_str, variation, demo_id, step_id, save_dir)
    ...
    def _save_current_frame(self, rgb_tensor, task_str, variation, demo_id, step_id, save_dir):
        """Save current RGB frames for all camera views."""
        try:
            # rgb_tensor shape: (1, num_cameras, 4, H, W) — contains RGB + attention channel
            rgb_data = rgb_tensor[0]  # remove batch dimension → (num_cameras, 4, H, W)

            for cam_idx in range(rgb_data.shape[0]):
                # Extract RGB channels only (exclude attention)
                rgb_image = rgb_data[cam_idx, :3]  # (3, H, W)

                # Convert tensor to NumPy and reorder dimensions
                rgb_np = rgb_image.detach().cpu().numpy().transpose(1, 2, 0)  # (H, W, 3)
                ...
                filename = f"{task_str}_var{variation}_demo{demo_id}_step{step_id:03d}_cam{cam_idx}.png"
                filepath = os.path.join(save_dir, filename)

                # Save RGB image
                Image.fromarray(rgb_np).save(filepath)

            # Print summary once at the first step
            if step_id == 0:
                print(f"Saved {rgb_data.shape[0]} camera frames to: {save_dir}")

        except Exception as e:
            print(f"Error occurred while saving current frame: {e}")
