...
class RLBenchDataset(Dataset):
    ...
    def __init__(
        ...
        for root, (task, var) in itertools.product(self._root, taskvar):
            ...
            if not data_dir.is_dir():
                ...
                continue
            npy_episodes = [(task, var, ep, extract_episode_id(ep.name)) for ep in data_dir.glob("*.npy") if ep.exists()]
            dat_episodes = [(task, var, ep, extract_episode_id(ep.name)) for ep in data_dir.glob("*.dat") if ep.exists()]
            pkl_episodes = [(task, var, ep, extract_episode_id(ep.name)) for ep in data_dir.glob("*.pkl") if ep.exists()]
    ...
    def __getitem__(self, episode_id):
        ...
        the episode item: [
            ...
            [imagine_imgs],  # wrt frame_ids, (3, H, W)
            [imagine_pcds],  # wrt frame_ids, (3, H, W)
            [transform_matrixes],  # wrt frame_ids, (4, 4)
        ]
        """
        episode_id %= self._num_episodes
        task, variation, file, id = self._episodes[episode_id]

        # Check if file exists before loading
        if not file.exists():
            print(f"Warning: File not found: {file}, trying next episode...")
            # Try next episode instead of returning None
            return self.__getitem__((episode_id + 1) % self._num_episodes)
            
        # Load episode
        episode = self.read_from_cache(file)
        if episode is None:
            print(f"Warning: Failed to load episode from {file}, trying next episode...")
            return self.__getitem__((episode_id + 1) % self._num_episodes)
        ...
        rgbs = states[:, :, 0]
        pcds = states[:, :, 1]
        rgbs = self._unnormalize_rgb(rgbs) # (B, ncam, 3, H, W)

        B, ncam, C, H, W = rgbs.shape
        goal_rgb = episode[6]
        goal_pcd = episode[7]

        if isinstance(goal_pcd, np.ndarray):
            goal_pcd = torch.from_numpy(goal_pcd)
        if isinstance(goal_rgb, np.ndarray):
            goal_rgb = torch.from_numpy(goal_rgb)

        goal_rgb = goal_rgb.unsqueeze(0).unsqueeze(0).repeat(B, 1, 1, 1, 1)
        goal_pcd = goal_pcd.unsqueeze(0).unsqueeze(0).repeat(B, 1, 1, 1, 1)
        ...
        edge_mask = torch.zeros(n_frames, dtype=torch.long)  # (n_frames,)
        ...
        ret_dict = {
            ...
            "edge_mask": edge_mask,
            "matrix": torch.from_numpy(matrix).float().unsqueeze(0).repeat(len(frame_ids), 1, 1)  # (n_frames, 4, 4)    
        }
        if self._return_low_lvl_trajectory:
            ret_dict.update({
